...
const Careers = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setDataLoading(true);
        const response = await axios.get("/data/jobData.json"); // Adjust path as needed
        const { jobOpenings: jobs, applicationSteps: steps } = response.data;
        
        // Map icon strings to actual React components
        const jobsWithIcons = jobs.map(job => ({
          ...job,
          icon: getIconComponent(job.icon)
        }));
        
        setJobOpenings(jobsWithIcons);
        setApplicationSteps(steps);
      } catch (error) {
        console.error("Error fetching job data:", error);
        showNotification("Failed to load job data. Please try again later.", "error");
      } finally {
  ...
  const getIconComponent = (iconName) => {
    const iconMap = {
      MdSchool: <MdSchool className="text-3xl" />,
      MdWork: <MdWork className="text-3xl" />,
      MdGroups: <MdGroups className="text-3xl" />
    };
    return iconMap[iconName] || <MdWork className="text-3xl" />;
  };
  ...
  const ApplicationModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div className="relative w-full max-w-2xl overflow-y-auto bg-white rounded-2xl shadow-2xl">
          {/* Header */}
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div>
              <label className="block mb-2 text-sm font-medium text-gray-700">
                Resume/CV + Other testimonials all in one PDF *
              </label>
  ...
  if (dataLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <FaSpinner className="mx-auto mb-4 text-4xl text-primary animate-spin" />
